...
int spa_convert_token_to_machine_type() {
	switch (token->type) {
		...
		case TTYPE_OPERATOR:
			...
			if( strcmp(token->lexeme, "?")  == 0 )   return MTTYPE_QUESTION_MARK;
			break;
		case TTYPE_ID:
			return MTTYPE_ID;
			break;
...
int machine_call_for_current_machine(int machine_token) {
	int next_machine;
	for (next_machine = 0; next_machine <  MACHINE_TYPES; next_machine++) {
		if(current_machine.machine_calls[current_machine.current_state][next_machine] != MACHINE_INVALID_STATE)
			if(machines_array[next_machine].state_transition_table[0][machine_token] != MACHINE_INVALID_STATE)
				return next_machine;
	}
